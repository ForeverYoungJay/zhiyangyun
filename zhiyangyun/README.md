...
- `GET /api/v1/b1-miniapp/requests?page=...&page_size=...&keyword=...&status=...`
- `POST /api/v1/b1-miniapp/requests/{id}/status?status=accepted|processing|completed`
- `GET /api/v1/b2-family/accounts?page=...&page_size=...&keyword=...&relation=...`
- `GET /api/v1/b2-family/elders/{elder_id}/bills?page=...&page_size=...&keyword=...&status=...`
- `GET /api/v1/b2-family/elders/{elder_id}/care-records?page=...&page_size=...&keyword=...`
- `GET /api/v1/b2-family/elders/{elder_id}/orders?page=...&page_size=...&status=...`
- `GET /api/v1/b2-family/elders/{elder_id}/balance-changes?page=...&page_size=...`
- `GET /api/v1/b2-family/surveys?elder_id=...&page=...&page_size=...`
- `POST /api/v1/b2-family/surveys`
- `GET /api/v1/b2-family/notifications?page=...&page_size=...&keyword=...`
- `GET /api/v1/b3-dashboard/metrics?page=...&page_size=...&start_date=...&end_date=...`
- `GET /api/v1/b3-dashboard/performance-summary`（新增手工补录联动字段）
...
- `A2-OA2 审批流`：多级审批状态机、抄送名单、审批轨迹
- `A2-OA3 通知触达`：触达策略（立即/队列）、送达/重试/失败动作
- `A2-OA4 培训闭环`：培训计划 + 签到 + 考核 + 闭环统计
...
- `2026021121_oa2oa4_business_upgrade.py`
  - OA2 审批单新增状态机字段（步骤、抄送、时间戳）与审批动作日志表
  - OA3 通知新增触达策略/状态/重试/目标用户字段
  - OA4 培训新增讲师与计划时间、签到/考核闭环字段
