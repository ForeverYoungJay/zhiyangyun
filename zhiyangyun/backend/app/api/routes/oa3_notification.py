...
def list_messages(page: int = 1, page_size: int = 10, keyword: str = "", status: str = "", channel: str = "", db: Session = Depends(get_db), current: CurrentUser = Depends(get_current_user)):
    return ApiResponse(data=service.list_notifications(db, current.tenant_id, page=page, page_size=page_size, keyword=keyword, status=status, channel=channel))
