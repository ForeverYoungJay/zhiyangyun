...
## 2026-02-24
### Added
- OA3 消息列表新增 `strategy/receiver_scope` 筛选；动作补齐状态机约束（pending/retrying/failed/sent）。
- OA4 培训记录补齐 `required_score` 返回字段，前端支持按课程筛选。

### Changed
- OA4 考核逻辑改为按课程及格线 `required_score` 判定，并要求先签到再考核。
- OA3/OA4 路由统一补齐 400 异常透出，前端筛选条件与后端参数对齐。
- 回归脚本补充 OA3 策略+范围筛选与重试状态机断言、OA4 及格线断言。

### Fixed
- 修复 OA4 评估通过阈值硬编码 60 导致的闭环判定错误。

### Docs
- 更新 README、`docs/04-API清单.md`、`docs/08-测试与验收手册.md`，同步 OA2~OA4 最新能力与回归口径。
...
- OA2/OA3/OA4 全部列表统一为分页+搜索+筛选，前端统一显示姓名字段替代纯 ID。

### Added
- OA1 发布动作自动联动 OA2：创建 `module=oa1_shift` 审批待办。
- 回归脚本补充 OA1 校验点：冲突检测、审批联动、日期区间筛选、姓名化字段。
- OA2 审批升级：支持多级状态机（pending/in_review/approved/rejected/cancelled）、抄送名单 `cc_user_ids`、审批轨迹 `approval_action_logs`。
- OA3 通知升级：新增触达策略（`immediate/queued`）与触达动作（`deliver/retry/fail`）。
- OA4 培训升级：新增培训计划时间/讲师、签到与考核动作、课程闭环统计。
- 新增迁移 `2026021121_oa2oa4_business_upgrade.py`。

### Docs
- 重写 `docs/08-测试与验收手册.md`：补充执行模式、判定标准、手工验收顺序、可复用验收模板。
- 新增本文件“变更日志模板”，统一后续记录格式。
- 更新 `docs/04-API清单.md`：补充 OA2/OA3/OA4 API、状态机与闭环说明。
