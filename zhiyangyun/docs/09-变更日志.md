...
## 2026-02-24
### Added
- OA1 新增排班状态流转接口：`POST /oa1-shift/assignments/{id}/status`（发布/执行/异常/重开）
- API 回归脚本新增 OA1 关键流程校验（分页、姓名化、状态流转）

### Changed
- OA1 模板/排班列表统一升级为分页+搜索+筛选
- OA1 排班列表返回 `user_name/username/shift_name`，前端改为姓名化展示替代纯 ID
- README 与 API/测试文档同步补充 OA1 业务化说明
